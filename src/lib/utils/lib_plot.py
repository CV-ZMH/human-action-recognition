...
def draw_frame_info(img, color='red', **kwargs):
    ...
    y0, dy = 20, 24
    color = (0, 255, 0) if color == 'green' else (0, 0, 255) if color == 'red'  else (255, 0, 0)
...
def draw_action_result(img_display, id, skeleton, str_action_label):
    ...
    img_display = cv2.putText(
        img_display, "P"+str(id)+": "+str_action_label, (TEST_COL, TEST_ROW), font, fontsize, (0, 0, 255), linewidth, cv2.LINE_AA)
